...
Факти для наступних задач (reusable-частина винесена в [[Knowledge/OpenCart3/INDEX|Knowledge]]).

- Верстку звіряти з `git show main:<файл>` (sections/*, helper/general.php — іконки, контакти).
- Патерн сторінок: route-контролер + модулі `<page>_*` через layout ([[Knowledge/OpenCart3/modules|modules]]); About-layout 14 ділять виробник і дилери.
...
- Наступні етапи (з handoff 2026-08-07): OTP email-код як shokeru (фронт видобутий, бекенд писати — [[Knowledge/OpenCart3/ajax|ajax]]); СЕО-фільтри/пагінація/сорт у path без GET.
