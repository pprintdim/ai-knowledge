...
| Тип запису | Місце |
|---|---|
| Про конкретний repo (память/рішення/задачі) | `Projects/{repo}/` (PROJECT, MEMORY, DECISIONS, TASKS, ARCHITECTURE за потреби) |
| Reusable знання | `Knowledge/{технологія}/` |
...
Існуючі .md у репо (включно з legacy `handoff.md`) не видаляти; читати як джерело на старті сесії; змінювати — тільки якщо потрібно для задачі/дозволено. НОВІ session-state записи ведуться тут у `Projects/{repo}/` (MEMORY+TASKS), не в нових handoff.md.

CLAUDE.md у репо не створювати автоматично: глобальні інструкції + MCP + `Projects/{repo}/` покривають контекст. Якщо без локального CLAUDE.md технічно ніяк — спершу повідомити користувача.
