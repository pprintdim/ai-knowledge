...
- **ЧПУ на nginx (2026-09-13)**: у vhost `/etc/nginx/sites-enabled/shoker.in.ua.conf` стояло `try_files $uri $uri/ /index.php?$args;` — `.htaccess` nginx не читає, усі слаги падали на головну. Виправлено як у shokeru: `try_files $uri $uri/ @opencart;` + `location @opencart { rewrite ^/(.*)$ /index.php?_route_=$1 last; }` (бекап у `/root/vhost-backups/`). **Оновлення CloudPanel може перезаписати vhost — перевіряти після апдейту.**
- Route-сторінки без сутності (contacts/rating/promo/info/shop/cart/checkout/account/*) — декодує стоковий `startup/seo_url.php` (рядок seo_url з `query=route`), генерацію додано кастомним блоком у `rewrite()`; посів — `catalog/view/theme/shoker/route_slugs.sql` (один keyword на всі мови). Слаг не має збігатися з текою web-root.
