...
- 2026-09-16 (UX): ролик плеєра «як це працює» (20 МБ) вантажився при відкритті сторінки товару — тепер `data-lazy-src` + IntersectionObserver у home.js. Сторінка товару: 21 МБ → 0.
- Зони дотику 44×44 накладкою `::after`. `.hp-card__wish` накладку зрізав `overflow: hidden` медіа-блоку, тож збільшено сам бокс (44×44, top/right 5px) + `@media (hover:none)` робить кнопку видимою на тачі (раніше вона була прозора без ховера).
- Поле телефону на контактах: додано `inputmode="tel"` і `autocomplete`.
- 2026-09-16: `storage` перенесено за межі веб-кореня → `/home/hydrophobnet/storage/` (у config.php і hp_panel/config.php абсолютний шлях). Стара тека видалена, ззовні 404, попередження в адмінці зникло.
