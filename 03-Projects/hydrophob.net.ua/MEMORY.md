...
- 2026-09-16 (UX): зони дотику 44×44 накладкою `::after`; чекаут підсвічує поля, які сервер повернув у `json.error` (клас `is-invalid`, підсвітка зникає при правці); стрілки галереї товару отримали `aria-label`.
- Стан «Додано»/«В кошику» на кнопках уже працював (через `common/cart/ids` + `hpMarkInCart`) — перевіряти перед тим, як робити наново.
- 2026-09-16: `storage` перенесено за межі веб-кореня → `/home/hydrophobnetua/storage/` (у config.php і hp_panel/config.php абсолютний шлях). Стара тека видалена, ззовні 404, попередження в адмінці зникло.
